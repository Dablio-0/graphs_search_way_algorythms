...
| **Busca em Profundidade Limitada** | DFS com limite de profundidade. Evita loops infinitos. | O(V + E) no pior caso; O(b^l) se o limite l for menor que a profundidade máxima |
| **Busca por Aprofundamento Iterativo (IDDFS)** | Executa DFS limitada com limite crescente. Combina vantagens de BFS e DFS. | O(b^d) em árvores; aproximado O(V + E) em grafos com marcação de nós visitados |
| **Busca Bidirecional** | Executa busca simultaneamente do início e do fim. Reduz o espaço de busca pela metade. | O(b^(d/2)) em árvores; aproximado O(V + E) em grafos |
